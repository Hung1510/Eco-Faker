...
## Shipped: data versioning/lineage (`version save/list/diff/branch/log`) (2026-07-26)

First real slice of the "data versioning/lineage" item from the original external backlog review -- picked over the review's other unstarted items (Great Expectations export, DB snapshot/restore with anonymization, live-streaming-as-a-service, OpenAPI-examples mocking, an interactive schema-designer GUI, BDD/Gherkin integration) specifically because it composes directly on top of machinery this project already has -- `--snapshot`'s recipe format, `replay`, `warp`, and `diff` -- rather than starting cold, and because it's the most immediately demoable and useful of the group day to day.

**Design bet, stated plainly:** a version stores only the *recipe* (`config` + `referenceNow`, the exact shape `--snapshot` already writes), never the generated dataset itself. This is the same bet `replay`/`warp` already made, extended one layer further -- a store with hundreds of named versions stays kilobytes, not gigabytes, at the cost of `version diff` needing to regenerate both datasets on every call rather than comparing cached data. For this tool's actual scale (hundreds-to-low-thousands of rows, deterministic seeded generation that's already fast), that trade is a clear win; it would not be for a tool whose generation step was slow or non-deterministic.

**What "branch" actually does, concretely:** `version branch <parent> <name> [flags]` copies `<parent>`'s saved config, layers any explicit flags/`--scenario`/`--scenario-file` on top via the exact same `mergeOverrides` precedence chain every other override in this tool already follows (explicit flags win), records `<parent>` for lineage, and defaults to a fresh `referenceNow` (i.e. "start this experiment from now") unless `--keep-reference-now` asks to reproduce the parent's exact point in time with only the config changed. `version log <name>` walks parent pointers back to the root and prints the chain in root-first order, with real cycle detection (a hand-edited store could produce one; this tool's own writes never can, since a parent must already exist before a child references it).

**Scope, stated the same way this project always states it:** this is a flat named store with lineage tracking, not a general version-control system -- no merge/rebase/conflict resolution, because there's genuinely nothing to conflict (nothing here is ever generated by more than one recipe at once, unlike a real VCS working tree). A version name is validated as a plain identifier (letters/numbers/`.`/`_`/`-`) specifically to block path traversal through the name argument, confirmed directly (`version save "../../evil"` rejected, nothing written outside the store dir) rather than assumed safe from reading the regex.

New `src/version-store.ts` (the persistence layer: save/load/list/lineage, no CLI concerns) plus five new subcommands under `version` in `src/cli.ts`, mirroring the existing `scenario resolve <file>` nested-subcommand pattern rather than introducing a new one. 22 new tests across `tests/version-store.test.ts` (the module in isolation, real fs in a temp dir) and `tests/version-cli.test.ts` (real end-to-end runs against the actual built `dist/cli.js`, since the flag-wiring and override-precedence live in `cli.ts` itself and a module-level test alone wouldn't catch a bug there) -- including confirming branch's override actually changes the regenerated dataset's real row counts (not just that the flag was accepted), and that an unknown name's error message lists what IS available rather than a generic failure.

`.eco-faker/` added to this project's own `.gitignore` -- it's per-project user data a `version save` creates in whatever directory it's run from, not something this repo's own checkout should ever risk committing from manual local testing.

## Shipped: OpenAPI-examples mocking (`serve --openapi-examples`) (2026-07-26)

Second slice of the original external backlog review's unstarted items -- picked over the remaining ones (Great Expectations export, DB snapshot/restore with anonymization, live-streaming-as-a-service, an interactive schema-designer GUI, BDD/Gherkin integration) for the same reason `version` was picked last round: it composes on top of what already exists (`serve`'s own Express app, auth/chaos middleware, and this project's established local-file-or-live-URL OpenAPI loading pattern from `init --schema`) rather than starting cold.

**What it actually is, concretely:** an entirely separate mode of `serve` (`--openapi-examples <path-or-url>`), not a variation on the existing dataset-backed mock. When given, dataset generation is skipped entirely -- every dataset-shaping flag (`--users`, `--seed`, `--scenario`, ...) is silently ignored, stated plainly in both the CLI help text and the README, not left to be discovered by confusion. New `src/openapi-mock.ts` holds the whole thing: a narrow, purpose-scoped `OpenApiExampleDoc` type (matching this project's existing convention of each module defining only the fields it needs, e.g. `OpenApiDocument` in `contract-test.ts`), example resolution (`example` -> `examples` map, alphabetically-first key -> `schema.example`), and the Express app builder.

**Scope, stated plainly:** OpenAPI 3.x only. Swagger 2.0 declares examples in a genuinely different shape (`responses.<code>.examples.<mediatype>`, no `content` wrapper) -- silently half-supporting both felt worse than being explicit that a 2.0 document's examples simply won't be found, falling back to the same honest "no example declared" 501 every other example-less route gets. This is also a stateless, single-happy-path mock: it has no way to know from a request alone which of several documented responses for one operation (a 200 vs a 404 for the same path) a given call wants, so it always serves the lowest declared 2xx (or `default`), never a request-aware state machine.

**A real bug, caught by actually running a server and requesting a real path -- not by reading the registration code:** Express matches routes in registration order. Registering every "has an example" route before every "declared but no example" route meant a param route like `/widgets/{id}` -> `/widgets/:id` silently swallowed a static sibling path like `/widgets/no-example`, which should have gotten its own distinct 501. Confirmed directly: the sibling returned the param route's 200 body instead. Fixed by combining both route lists into one and sorting by specificity (fewer `:param` segments first, longer path as a tiebreak) before registration, so static routes always win over param routes for the same method regardless of which of the two lists either one came from -- re-verified against a real running server after the fix, both in the module's own tests and manually with a live process on a real port.

`authMiddleware`/`chaosMiddleware` exported from `serve.ts` rather than reimplemented -- confirmed directly (no header -> 401, wrong key -> 401, correct key -> 200) against a real running server, not assumed to work from the import alone.

16 new tests in `tests/openapi-mock.test.ts` (real Express app, real `listen(0)`, real `fetch()` -- the same pattern `tests/chaos-auth-openapi-live.test.ts` already established for the dataset-backed server) covering example resolution priority, the alphabetically-first `examples` key, the 501/404 distinction, the route-collision fix specifically, and auth reuse. Also a new CI e2e step in the `mock-api-e2e` job, matching the existing serve/chaos/auth/postman steps' exact shape: spins up a real `serve --openapi-examples` process and curls it for the same three behaviors (real example served, static-sibling-not-swallowed, unknown-route 404).

## Shipped: Great Expectations export (`ge-export`) (2026-07-26)

Third slice of the original external backlog review, picked over the remaining unstarted items (DB snapshot/restore with anonymization, live-streaming-as-a-service, an interactive schema-designer GUI, BDD/Gherkin integration) as the narrowest, most self-contained one -- structurally almost identical to `benchmark-export`'s existing shape (one artifact per table, `--input`-or-generate, no new CLI conventions needed), and genuinely free to run (no external API, no cost) per an explicit standing constraint on this project going forward that no new feature should require a paid/metered dependency to work at all.

**Verified the real, current GE JSON shape before writing anything** -- `expectation_suite_name`/`data_asset_type`/top-level `meta`, each expectation as `expectation_type`+`kwargs`+`meta` -- against Great Expectations' own docs and several real published suite examples, rather than assumed from training-data memory of a tool that's had multiple format-relevant releases.

**Every expectation is derived from real observed values in the actual generated dataset, never a hardcoded assumption** -- reusing `datasetToCanonicalRows` (the same real source of truth `benchmark-export`'s Elasticsearch mapping already relies on): column existence/order from the columns genuinely present, not-null only where literally every real row has one (a legitimately-nullable field like `support_tickets.resolvedAt` on an unresolved ticket correctly gets no such expectation -- confirmed directly, not assumed), numeric range from the real min/max, and an enum-style `value_set` only for string columns whose distinct-value count is small relative to row count.

**A real quality bug caught before shipping, not after:** the uniqueness check was originally proposed for every column (of any type) that happened to hold unique values in the sample -- which flagged `orders.total`, a continuous float, as "must be unique," purely because a 48-row sample of dollar amounts didn't happen to collide. That's a statistical accident of small continuous values, not a real business invariant the way an id/sku's uniqueness is -- nothing stops two real orders from sharing a total. Fixed by excluding float-typed columns from the uniqueness check entirely, confirmed by regenerating and inspecting the actual suite before and after.

**Tests, not just typechecking:** `tests/great-expectations.test.ts` includes a multi-seed regression that doesn't just check the generation code's internal consistency -- for every expectation emitted across 5 different seeds, it independently re-derives the real data and asserts the expectation's claim is actually true of it (every not-null column really has none, every unique column really has no duplicates, every range/enum bound really holds), which is the property that actually matters for a suite meant to validate real data. Also directly regenerates a real dataset via the built CLI and inspects the written JSON files by hand (not just through the library functions) to confirm `id`/`total`/`status`/`currency`/`resolved_at` all come out the way the design intends.

No new CI e2e step -- `benchmark-export`, the closest existing analog, also has none (unit tests alone cover it), so this follows the same precedent rather than introducing a new one.

## Shipped: SSE live feed (`serve --live-sse`) (2026-07-26)

A second transport for the existing WebSocket live feed (`--live`), not a new feature from scratch -- `GET /live/sse` (Server-Sent Events) broadcasts the exact same event list at the exact same cadence, for any client/environment that can do a plain HTTP GET but not a WebSocket upgrade: `curl -N`, a browser's built-in `EventSource`, or a restrictive corporate proxy/serverless/API-gateway setup that blocks WS outright but passes through a long-lived HTTP response fine. Picked as the scoped-down first slice of the original backlog's "live-streaming-as-a-service" item once it became clear the WebSocket feed already covers the core of what that phrase implies -- the real gap was transport compatibility, not missing streaming capability.

**A real routing-order bug caught by actually testing, not by reading the registration code -- the same category of bug `serve --openapi-examples` caught last round, in the same file family:** the first implementation called `attachLiveFeedSSE(app, ...)` on the `Express` app *after* `createMockApiServer` had already returned it. But `createMockApiServer` registers a catch-all 404 handler (`app.use((req, res) => res.status(404)...)`) as the very last thing before returning -- any route added to `app` afterward sits after that catch-all in Express's registration order and is never reached, no matter what path it declares. Confirmed directly: the first real request to `/live/sse` came back 404. Fixed by threading the original `overrides`/`referenceNow` (which `buildWebhookEvents` needs to regenerate its event list -- `createMockApiServer` only receives the already-built `Dataset`, not the recipe that made it) into a new `ServeOptions.liveSse` field, and mounting the SSE route *inside* `createMockApiServer` itself, immediately before its own catch-all -- the exact same place `--openapi`/`--postman`/`--graphql` already get mounted. Re-verified after the fix against a real running server: `200`, correct `text/event-stream` content type, real events, and confirmed `/api/*` routes are unaffected.

**Design choice stated plainly:** unlike the WebSocket feed (one shared broadcast loop iterating `wss.clients`), each SSE connection gets its own independent cursor and `setInterval` timer. There's no equivalently cheap way to fan out a single loop across many independent long-lived HTTP responses the way WS's client-iteration does, and at this mock server's realistic connection counts (a handful of local dev/test clients, not a production pub-sub system) that isn't worth the added complexity of building one.

5 new tests in `tests/chaos-auth-openapi-live.test.ts` (17/17 passing in that file total, including the pre-existing WebSocket feed test) -- covering real event streaming over HTTP, the correct content type, a dedicated regression test for the exact route-ordering bug above, the honest 404 when `liveSse` isn't configured, and confirming `/api/*` is unaffected. Also manually verified end-to-end against the actual built `dist/cli.js serve --live-sse` process, not just the library function in isolation.

## Shipped: DB snapshot + anonymization (`db-snapshot`) (2026-07-26)

The last of the original external backlog review's "narrower" unstarted items (leaving only an interactive schema-designer GUI and BDD/Gherkin integration, both genuinely bigger/fuzzier efforts, as the remaining unstarted work from that review). Picked once it became clear this composes directly on an existing pattern rather than starting cold: `lint.ts`'s `lintSqlAgainstDatabase` already establishes the exact "optional `pg` package, dynamic import, real live Postgres connection" shape this needed -- `db-snapshot.ts` reuses that shape verbatim rather than inventing a second way to optionally depend on `pg`.

**Verified against an actual live Postgres instance, not mocked** -- this sandbox can run real Postgres (`apt-get install postgresql`, confirmed reachable over real TCP on 127.0.0.1:5432), so unlike `lint.ts`'s own stated gap ("not exercised by this offline function... would require a live Postgres instance in CI"), this feature's core live-DB behavior was actually exercised end to end: a real schema (`users`/`orders`/`products`) seeded with real-shaped PII (names, emails, phones, SSNs, street addresses, dates of birth), snapshotted through the real built CLI, and the output inspected by hand.

**A real false positive, demonstrated against real data, not just anticipated in a comment:** the column-name heuristic that flags `products.name` as PII fired for real, and the real product names ("Wireless Mouse", "Mechanical Keyboard") came back replaced with fake PERSON names ("Sean Ankunding", "Hugh Barton") on the first real run. This is the exact, expected trade-off of a name-only heuristic (this tool has no way to distinguish a product-name column from a person-name column without reading the actual content, which is out of scope for a fast, generic first slice) -- but it's documented here as an observed fact from a real run, not a hedge. `--exclude-anonymize table.column` fixed it, confirmed by re-running against the same real data. `--anonymize table.column` (the reverse -- force an unflagged real PII column) and generic `password`/`secret`/`token` columns being redacted to an opaque marker rather than a plausible-looking fake credential (a real password hash should never come back looking like a valid one) were also confirmed against the real database.

**Determinism confirmed as an actual property of separate process runs, not assumed from the hash-based design:** the same real email, snapshotted twice in two independent calls, produced the exact same fake replacement both times -- this is what keeps a value repeated across tables (or across two separate snapshot runs of the same source data) consistent with itself in the anonymized output, without ever storing a reversible real-to-fake mapping anywhere.

**Scope, stated plainly:** `--row-limit` (default 1000/table, no `ORDER BY`) is a safety cap, not a representative sample or a referential-integrity guarantee -- a foreign key can point outside another table's own row limit on a large database. Output is JSON only for this first slice; a CSV or SQL-INSERT output format (for a more direct `\copy`/reseed workflow) is a natural next slice, not built here.

New `tests/db-snapshot.test.ts` (36 tests) covers every pure function -- `classifyColumn`'s full pattern set including the ordering-dependent first_name/last_name-vs-generic-name distinction, and `anonymizeValue`'s determinism, non-reversibility, and per-kind shape (real SSN pattern, real ISO date, the opaque redaction marker for secrets) -- and caught one real regression before it shipped: the `email` pattern didn't match the `e_mail` (underscore) naming variant, only `email`/`e-mail`. The live-database-querying `snapshotDatabase()` function itself is verified manually (as described above) rather than by the automated suite, for the same reason `lintSqlAgainstDatabase` isn't: no live Postgres in CI. `pg` remains a fully optional, undeclared dependency (not added to `package.json`), same as the existing `lint --db-url` feature.

## Shipped: BDD / Gherkin testing (`test --gherkin`) (2026-07-26)

The last of the original external backlog review's remaining items gets scoped down (leaving only the interactive schema-designer GUI as genuinely unstarted, plus the VS Code Miller-columns drill-down set aside earlier by choice, not by difficulty). Picked once it became clear the existing `test --scenario` engine (`runScenarioTest`, already real, already tested) is exactly the right execution engine underneath real Gherkin syntax -- this is a translation layer on top of proven machinery, not a second test-execution system.

**What it deliberately is, and isn't:** `src/gherkin.ts` parses a real `.feature` file's `Feature:`/`Scenario:`/`Given`/`When`/`Then`/`And`/`But` structure into the exact same `Scenario`/`ScenarioStep` shape `--scenario` already runs through `runScenarioTest` -- every step really fires a request and is really asserted, nothing is faked or simulated. It is NOT a general-purpose Gherkin/Cucumber runner with user-registrable step definitions (arbitrary custom code per step) -- that's a fundamentally different, much bigger feature. Instead it's a small, fixed step vocabulary (an HTTP call, a status assertion, a response-field assertion, a capture) -- enough to actually author real BDD-style API scenarios in real `.feature` syntax, not enough to replace Cucumber for general-purpose step authoring.

**Scope stated plainly, with a clear parse error rather than a silent misparse for each:** tags (`@smoke`), `Background:`, `Scenario Outline:`/`Examples:` (data-driven scenarios), and data tables/doc strings are all real, common Gherkin syntax this first slice doesn't support -- rejected loudly with a specific reason and a suggested workaround (repeat setup per-Scenario instead of Background; write out each case as its own Scenario instead of an Outline), rather than quietly ignoring the line and producing a scenario that doesn't test what the file's author actually wrote.

**Verified end-to-end against a real running mock API, not just at the parser level:** generated a real dataset, started a real `serve` process, produced a real OpenAPI contract via `openapi-export`, wrote a real two-`Scenario` `.feature` file, and ran `test --contract ... --gherkin ...` against it -- confirmed both scenarios pass for real (a real capture, a real 404 check), confirmed a deliberately-wrong assertion fails with the right message and the right process exit code (1, not swallowed), and confirmed the tricky `with body {...} and call it "..."` regex backtracking actually parses correctly rather than the body greedily swallowing the trailing clause.

27 new tests in `tests/gherkin.test.ts` -- every step pattern, every rejected-syntax error path, value-parsing edge cases (string/number/boolean/null), multi-scenario files, and an integration section that runs a parsed scenario through the real `runScenarioTest` with a mocked `fetch`, confirming both a passing and a failing case report correctly (not just that the types line up).

## Shipped: interactive schema-designer GUI (2026-07-26)

The last remaining item from the original external backlog review (leaving only the VS Code Miller-columns drill-down, set aside by choice earlier in this session, not by difficulty). Scoped down from "interactive schema-designer GUI" -- a phrase vague enough to mean several different things for this project -- via a direct clarifying question, which settled on two concrete, real things to build rather than guessing at a multi-hour scope: (1) toggle which of the 5 real optional data modules to include, with the equivalent config/CLI command shown live, and (2) a visual editor for the ORM scaffold's column mapping.

**Both pieces extend, rather than duplicate, the existing `web-static/` static-demo architecture** (the same "bundle real backend code client-side via esbuild, no server" approach `index.html`/`explorer.html` already established) instead of introducing a new app shape. Piece 1 extends the existing `index.html`/`app.ts` sliders demo directly. Piece 2 is a new page, `mapping-designer.html`/`mapping-designer.ts`, since it's a genuinely different workflow (pasting schema text, not generating a dataset) that would have cluttered the sliders demo.

**Piece 1 -- module toggles + live config/CLI output:** 5 checkboxes for the 5 real `EcoFakerConfig.*.enabled` flags (there is no finer-grained "which of the ~21 tables to include" toggle in the generator itself -- core tables like users/orders/carts are never independently disableable, and the UI is honest about that rather than implying control that doesn't exist) plus a catalog-size slider, live-rendering both a JSON overrides snippet and the real, current, copy-pasteable CLI flag names (`--no-recommendation-data` etc., confirmed against the actual `generate` command's real options, not invented shorthand).

**Piece 2 -- ORM mapping visual editor:** exports `parsePrismaSchema`/`buildSchemaMapping`/`CANONICAL_COLUMNS`/`matchColumn`/`CORE_TABLES` from `browser.ts` -- confirmed these are genuinely pure, dependency-free functions (no `node:fs`) by reading their source AND by confirming the whole `browser.ts` bundle still builds cleanly for a browser target via esbuild, not assumed safe. Paste a real `schema.prisma`, see the real `buildSchemaMapping` output with confidence-color-coded badges, override any column's target (or a whole table's target model, which correctly recomputes every column against the new model's real fields rather than leaving stale matches from the old one), and download the resulting `mapping.json` -- the exact artifact `init --schema`/`generate --mapping` already consume.

**Scope stated plainly:** Prisma schemas only in this first slice. `init drizzle`/`init sqlalchemy` are still templates on the CLI side for the same underlying reason (no schema parser built for either yet) -- this tool doesn't pretend to cover more than it does.

**Verification, real end to end:** both pieces got a real jsdom-based smoke test (`scripts/smoke-test-static.cjs` extended; new `scripts/smoke-test-mapping-designer.cjs`), running the actual esbuild-produced bundle inside a real DOM, not a hand-rolled fake one (this exact distinction already burned this project once before, per `smoke-test-static.cjs`'s own history) -- real user interactions (checking/unchecking toggles, editing a schema, overriding a mapping, clicking download) and real assertions on the resulting DOM/exported JSON, not just that the code runs without throwing. Both new CI-run smoke tests, plus the pre-existing one, are wired into the same `.github/workflows/ci.yml` step.

A real bug was caught and fixed in the test itself while getting this working, not in the app: an early version of the mapping-designer smoke test picked "the first dropdown option that differs from the current one" to test an override with, which happened to be the "(none -- skip this column)" placeholder (value `""`) -- and then asserted the exported mapping's `targetColumn` would literally become `""`. The app's actual behavior (treating an empty selection as "clear the override, revert to the real auto-match" rather than writing a literal empty string as a column name) is the correct one; the test was fixed to skip the placeholder and pick a real column to override with instead.

## Shipped: VS Code Miller-columns relationship drill-down (2026-07-26)

The very last item from the original external backlog review, previously set aside by choice (not difficulty) while other items got picked up first. A direct, faithful port of `web-static/explorer.ts`'s proven Miller-columns drill-down (User → Orders → Shipment/Returns) into the VS Code extension's existing table viewer -- not a new design, and not a rewrite of the flat table view that already ships. Differs from the browser demo in exactly one real way: no "regenerate" button, since the extension's webview shows a real, already-generated `dataset.json` a person picked via the file dialog, not a live, re-rollable `generate()` call.

**Integrated as a second view mode in the same webview**, not a second command/panel -- a "Tables" / "Relationships" tab toggle at the top of `eco-faker: View Dataset Tables`'s existing output, both tabs sharing the one embedded `DATASET` already loaded. This matches the roadmap's own original framing of this item ("a natural next layer on the table viewer that already ships") more literally than a separate command would have.

**Honest about when it applies:** the drill-down assumes a specific shape (non-empty `users`, `orders`, `shipments`; `returnRequests` optional). A dataset missing any of those gets a disabled "Relationships" tab with a tooltip explaining exactly why, rather than a tab that silently opens onto three empty columns with no indication anything's wrong -- computed for real from the actual loaded dataset's real array lengths, not assumed.

**Verified the same way the existing table viewer already was**, not a lesser bar for the new code: 12 new tests in `vscode-extension/tests/tableViewer.test.ts`, actually executing the real embedded client script via jsdom (the same technique already established here, and the same one this project's root repo uses for its own static-demo smoke tests) -- real clicks drilling user → order → shipment/return, real empty states for a user with no orders and an order with no shipment/return, the breadcrumb reflecting the real current path, the search filter genuinely filtering (not just visually hiding), HTML escaping in relationship fields, and both the enabled and disabled states of the Relationships tab. 37/37 extension tests pass (25 pre-existing + 12 new), the extension's own `tsc --noEmit` and `esbuild` bundle both run clean, and a real installable `.vsix` was produced via `vsce package` -- same verification bar the extension's README already documents for the rest of it.

**A real environment issue hit and fixed along the way, unrelated to the feature's own code:** the extension's `node_modules` had drifted to a mismatched `esbuild` binary/host version (a leftover from earlier `pg`/other package installs during this session's work on unrelated features), causing `esbuild.mjs` to fail outright with a version-mismatch error. Fixed with a clean `rm -rf node_modules && npm install` in `vscode-extension/`, confirmed by re-running the full typecheck/test/build/package sequence afterward.

With this, every item from the original external backlog review this session worked through is shipped: `version`, `serve --openapi-examples`, `ge-export`, `db-snapshot`, `test --gherkin`, the schema-designer GUI (module toggles + ORM mapping editor), and now this. The one item deliberately never started, per an earlier explicit decision in this same session: any feature requiring a paid/metered external API (the original `--prompt` natural-language generation was removed for exactly this reason, and no replacement was built).

## Shipped: file-based anonymization (`anonymize`) (2026-07-30)

The first of two enhancements picked once the original backlog review was fully cleared, chosen specifically because it composes on `db-snapshot`'s existing anonymization logic rather than starting cold: the much larger group of people with an export already (a CSV dump, a `dataset.json`) but no live database access can now get the exact same deterministic pseudonymization `db-snapshot` offers, without needing Postgres at all.

**A real refactor, not just a new file:** the classification/anonymization logic (`classifyColumn`, `anonymizeValue`, and a new shared `anonymizeRows`) moved out of `db-snapshot.ts` into a new `src/anonymize.ts`, with `db-snapshot.ts` now re-exporting them for backward compatibility rather than duplicating the implementation -- the exact anti-duplication principle this project applies everywhere else (`CANONICAL_COLUMNS`, `mergeOverrides`, etc.). `db-snapshot.ts`'s live-Postgres path now calls the same `anonymizeRows` the file-based command uses.

**A new, real, dependency-free CSV parser** (`parseCsv` in `anonymize.ts`) -- RFC 4180-shaped (quoted fields, `""` escaped quotes, commas/newlines inside quotes), since this project has no CSV-parsing dependency and the real quoting rules are small enough to implement and test directly rather than take one on. Its one real, confirmed-not-assumed scope limit: a bare quote appearing mid-*unquoted*-field (not valid RFC 4180 -- a field with a quote or comma must be quoted from its very first character) is silently lossy, not an error. This was discovered by writing a deliberately malformed test fixture during development, not anticipated in advance -- the fix was to write a *valid* fixture and confirm the parser handles it correctly, and to document the malformed case as a known, tested limitation rather than pretend it doesn't exist.

**A second, more serious real bug found by anonymizing an actual generated dataset, not assumed safe from the earlier `products.name` lesson:** `abandonedCheckouts.recoveryEmailSent` (a **boolean**) and `.recoveryEmailSentAt` (an ISO timestamp string) both matched `classifyColumn`'s `/e[-_]?mail/i` pattern (the substring "email" appears in both names) and got corrupted into fake email strings -- a boolean silently becoming a string is a genuine type corruption, worse than the earlier semantic-only near-miss. Fixed with a new `looksPlausibleForKind` check in `anonymizeRows`: an auto-detected (never a user-forced `--anonymize`) classification is rejected if the column's real values don't plausibly fit -- no PII kind is ever legitimately a boolean, and an "email" classification specifically requires the real values actually contain "@" (which also catches the ISO-timestamp case, since a date string has no "@"). Confirmed the fix against the same real dataset: the boolean/timestamp are now correctly left alone, while `users.email` still gets correctly anonymized. This improvement applies to `db-snapshot` automatically too, since both now share the same function.

New `tests/anonymize.test.ts` (23 tests) covers classification, the plausibility-check regression specifically (both the boolean case and the string-but-no-@ case), the escape hatches (`--anonymize`/`--exclude-anonymize`) still working as documented, and the CSV parser's real quoting behavior including the malformed-vs-valid distinction. `db-snapshot.test.ts`'s existing 36 tests still pass unchanged after the refactor. Manually verified end-to-end through the real built CLI: a real CSV with a properly-quoted comma and escaped quotes, and a real 21-table `dataset.json`.

## Shipped: k6 load-test script export (`k6-export`) (2026-07-30)

The second of the two post-backlog enhancements. A new artifact alongside `benchmark-export`/`ge-export`/the Postman export: a real, runnable [k6](https://k6.io/) load-test script, in two modes -- targeting eco-faker's own mock API (using the real, current `TABLE_ROUTES` from `serve.ts`, never a second hand-typed route list) or a real API matching a given OpenAPI contract (reusing `contract-test.ts`'s existing `OpenApiDocument` type, the same one `test --contract` already validates against).

**Scope, stated plainly, for contract mode:** only paths with zero path parameters are treated as list entry points, and a "detail" sibling is looked for by checking whether that same path with a single trailing `/{param}` also has a declared GET operation -- real ids for the detail check are discovered *live*, at k6 run time, from the list response's first row (the same "don't assume, capture a real one" principle `test --scenario`'s `{{step.field}}` captures already use), not read from any example value in the spec. Paths with more than one path parameter, or a parameterized path with no plain list-style parent, aren't attempted at all -- a real OpenAPI-to-k6 general path-parameter resolver (using declared examples, handling arbitrary nesting) is a substantially bigger, different feature than this first slice takes on.

**Verified far more rigorously than reading the generated JS and trusting it, because the real `k6` binary isn't installable here** (needs `dl.k6.io`, not on this environment's allowed network list) -- `apt-cache search k6` confirmed no package exists either. Instead: the *literal* generated script (not a reimplementation) was executed via Node's real `vm.SourceTextModule`/`vm.SyntheticModule`, providing real implementations of the `k6/http` and `k6` modules. The first version of this harness used Promise-based `fetch` for `http.get` -- which would have silently broken every check, since real k6's `http.get` is genuinely *synchronous* (blocks in Go under the hood) and the generated script never awaits it. Caught this before trusting any result, and rewrote the shim's `http.get` to be genuinely synchronous via `execFileSync` + `curl` -- a real, blocking network call, matching real k6 semantics rather than approximating them. With that fix: ran the actual generated script against a real `serve` process (42/42 real HTTP requests + checks passing across all 21 real routes, list+detail pairs with real ids threaded through), against a real API-key-protected server (auth header genuinely sent and accepted), confirmed a *missing*-auth run correctly produces real 401s and real check failures (not silently passing), and ran the real CLI's own `--contract` mode against a real generated OpenAPI contract end-to-end the same way.

14 new tests in `tests/k6-export.test.ts` cover the generation logic itself (real syntax validity via a fixed-up `new Function()` check, every real `TABLE_ROUTES` route present, vus/duration/baseUrl/apiKey all reflected correctly, contract-mode route derivation including the multi-path-param exclusion and the no-GET-operation exclusion) -- the live-network behavior above was verified manually per the paragraph before this, the same "state what's tested where, and why" approach already established for `lint.ts`'s live-DB path and the VS Code extension's Extension-Host gap.
