...
describe("recommendation data generation", () => {
  ...
  describe("behavioral flow: view -> wishlist -> purchase -> review", () => {
    ...
    it("review text is genuinely varied -- not the small fixed set of ~10-15 strings this used to be", () => {
      const dataset = generate({ seed: 3, scaleFactor: 500 });
      const withText = dataset.productRatings.filter((r) => r.reviewText !== null);
      expect(withText.length).toBeGreaterThan(50);
      const distinctTexts = new Set(withText.map((r) => r.reviewText));
      // A real, large sample of distinct review strings, not a small
      // repeating pool -- most reviews should be unique given the real
      // product name is interpolated into every one.
      expect(distinctTexts.size / withText.length).toBeGreaterThan(0.8);
    });

    it("review text references the real product name it's rating, not a generic placeholder", () => {
      const dataset = generate({ seed: 3, scaleFactor: 300 });
      const productsById = new Map(dataset.products.map((p) => [p.id, p]));
      const withText = dataset.productRatings.filter((r) => r.reviewText !== null);
      expect(withText.length).toBeGreaterThan(0);
      for (const rating of withText.slice(0, 20)) {
        const product = productsById.get(rating.productId)!;
        expect(rating.reviewText).toContain(product.name);
      }
    });
