...
describe("Elasticsearch benchmark export", () => {
  ...
  describe("mappings", () => {
    ...
    it("regression: known free-text columns (body, review_text, query, subject) always map to 'text', even when every sampled value happens to be short", () => {
      // These stayed under the 256-char length threshold in typical
      // generated data, so a length-only heuristic wrongly mapped them
      // exact-match "keyword" instead of full-text-searchable "text" --
      // backwards for data whose whole purpose is NLP/search training.
      expect(mappings.support_messages.mappings.properties.body.type).toBe("text");
      expect(mappings.support_tickets.mappings.properties.subject.type).toBe("text");
      expect(mappings.product_ratings.mappings.properties.review_text.type).toBe("text");
      expect(mappings.search_queries.mappings.properties.query.type).toBe("text");
    });

    it("enum-shaped fields (category, status, priority, sender) still map to keyword, not text", () => {
      expect(mappings.support_tickets.mappings.properties.category.type).toBe("keyword");
      expect(mappings.support_tickets.mappings.properties.status.type).toBe("keyword");
      expect(mappings.support_tickets.mappings.properties.priority.type).toBe("keyword");
      expect(mappings.support_messages.mappings.properties.sender.type).toBe("keyword");
    });
  });
