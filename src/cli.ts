...
import { createOpenApiExampleServer, loadOpenApiExampleDoc } from "./openapi-mock.js";
...
addCoreGenerateOptions(
  ...
  .option(
    "--openapi-examples <path-or-url>",
    "instead of a generated dataset, serve real response examples straight from an OpenAPI 3.x document (local .json/.yaml/.yml file, or a live http(s):// URL) -- every declared path+method with an example/examples/schema.example becomes a real route; --users/--seed/--scenario/etc. and every dataset-only option below are ignored in this mode. See README's 'OpenAPI-examples mocking' section."
  )
  .action(async (opts) => {
    if (opts.openapiExamples) {
      const port = opts.port as number;
      let doc: Awaited<ReturnType<typeof loadOpenApiExampleDoc>>;
      try {
        doc = await loadOpenApiExampleDoc(opts.openapiExamples as string);
      } catch (err) {
        console.error((err as Error).message);
        process.exit(1);
        return;
      ...
      const chaos = opts.chaos
        ...
        : undefined;
      const app = createOpenApiExampleServer(doc, {
        chaos: chaos && Object.keys(chaos).length > 0 ? chaos : opts.chaos ? true : undefined,
        apiKey: opts.apiKey,
        quiet: Boolean(opts.quiet),
      });
      app.listen(port, () => {
        console.log(`OpenAPI-examples mock running at http://localhost:${port}`);
        console.log(`  GET http://localhost:${port}/  (which routes have/don't have a usable example)`);
        if (opts.chaos) console.log(`  chaos mode ON: latency/500/429 injected into every response`);
        if (opts.apiKey) console.log(`  auth ON: send "Authorization: Bearer ${opts.apiKey}" or every request gets a 401`);
        if (!opts.quiet) console.log(`  request log ON: method/path/status printed per request (--quiet to silence)`);
      });
      return;
    }
...
/**
 * A local, named store of dataset recipes (config + referenceNow -- the
 * same shape `generate --snapshot` already writes), so a run can be saved
 * under a memorable name instead of a file path, diffed against another
 * saved version by name, or used as the starting point for a new version
 * ("branch") whose lineage is tracked. Doesn't store generated data itself
 * -- everything here regenerates from a small recipe, the same bet
 * `--snapshot`/`replay`/`warp` already made. Deliberately scoped to a flat
 * named store (no merge/rebase/conflict resolution -- there's nothing to
 * conflict, since nothing is ever generated by more than one recipe at
 * once): a real first slice of "data versioning/lineage," not the whole
 * of what a version-control system could mean applied to datasets.
 */
const versionCommand = program
  .command("version")
  .description(
    "A local, named store of dataset recipes -- save a version, branch a new one from it, diff two by name, list what's saved, or show a version's lineage. See README's 'Data versioning' section."
  );
...
addCoreGenerateOptions(
  versionCommand
    .command("save <name>")
    .description(
      "Save a new root version (no parent) under <name> -- either freshly generated from the flags below, or loaded from an existing `generate --snapshot` recipe via --snapshot."
    )
)
  .option("--snapshot <path>", "load an existing .snapshot.json recipe instead of building one from the flags below")
  .option("--message <text>", "a short note describing this version")
  .option("--dir <path>", `version store directory (default: ${DEFAULT_VERSION_STORE_DIR})`, DEFAULT_VERSION_STORE_DIR)
  .action((name: string, opts) => {
    const storeDir = path.resolve(process.cwd(), opts.dir as string);
    let referenceNow: number;
    let config: Partial<EcoFakerConfig>;
    if (opts.snapshot) {
      const snapshot = JSON.parse(readFileSync(path.resolve(process.cwd(), opts.snapshot as string), "utf-8")) as Snapshot;
      referenceNow = snapshot.referenceNow;
      config = snapshot.config;
    } else {
      referenceNow = Date.now();
      config = resolveOverrides(opts);
    }
    try {
      saveVersion(storeDir, {
        name,
    ...
    console.log(`Saved version "${name}" to ${storeDir}${opts.message ? `: ${opts.message}` : ""}.`);
  });

versionCommand
  .command("list")
  .description("List every saved version, oldest first.")
  .option("--dir <path>", `version store directory (default: ${DEFAULT_VERSION_STORE_DIR})`, DEFAULT_VERSION_STORE_DIR)
  .action((opts) => {
    const storeDir = path.resolve(process.cwd(), opts.dir as string);
    const versions = listVersions(storeDir);
    if (versions.length === 0) {
      console.log(`No versions saved yet in ${storeDir}. Run \`version save <name>\` first.`);
      return;
    }
    for (const v of versions) {
      console.log(`${v.name.padEnd(24)} ${v.createdAt}  parent=${v.parent ?? "(none)"}${v.message ? `  ${v.message}` : ""}`);
    }
  });
...
versionCommand
  ...
  .description(
    "Structurally diff two saved versions by name (row counts, schema drift, status-distribution shifts) -- same report as `diff`, sourced from the store instead of file paths."
  )
  .option("--fail-on-schema-change", "exit with code 1 if any table's field set differs between A and B")
  .option("--dir <path>", `version store directory (default: ${DEFAULT_VERSION_STORE_DIR})`, DEFAULT_VERSION_STORE_DIR)
  .action((nameA: string, nameB: string, opts) => {
    const storeDir = path.resolve(process.cwd(), opts.dir as string);
    let a: VersionRecord;
    let b: VersionRecord;
    try {
      a = loadVersion(storeDir, nameA);
      b = loadVersion(storeDir, nameB);
    } catch (err) {
      console.error((err as Error).message);
      process.exit(1);
      return;
    }
    const report = diffDatasets(materializeVersion(a), materializeVersion(b));
    console.log(formatDiffReport(report, nameA, nameB));
    if (opts.failOnSchemaChange && report.hasSchemaChanges) {
      console.error("\nSchema drift detected -- failing as requested by --fail-on-schema-change.");
      process.exit(1);
    }
...
addCoreGenerateOptions(
  versionCommand
    .command("branch <parent> <name>")
    .description(
      "Save a new version under <name> whose config starts as a copy of <parent>'s -- explicit flags below (and --scenario/--scenario-file) override the parent's values, the same precedence every other override already follows. Records <parent> for lineage (see `version log`)."
    )
)
  .option("--message <text>", "a short note describing this version")
  .option(
    "--keep-reference-now",
    "reuse the parent's exact referenceNow instead of a fresh one -- for reproducing the parent's dataset exactly plus config changes, rather than starting from a new point in time"
  )
  .option("--dir <path>", `version store directory (default: ${DEFAULT_VERSION_STORE_DIR})`, DEFAULT_VERSION_STORE_DIR)
  .action((parent: string, name: string, opts) => {
    const storeDir = path.resolve(process.cwd(), opts.dir as string);
    let parentRecord: VersionRecord;
    try {
      parentRecord = loadVersion(storeDir, parent);
    } catch (err) {
      console.error((err as Error).message);
      process.exit(1);
      return;
    }
    const config = mergeOverrides(parentRecord.config, resolveOverrides(opts));
    const referenceNow = opts.keepReferenceNow ? parentRecord.referenceNow : Date.now();
    try {
      saveVersion(storeDir, {
        name,
        message: opts.message as string | undefined,
    ...
    console.log(`Saved version "${name}" (branched from "${parent}") to ${storeDir}${opts.message ? `: ${opts.message}` : ""}.`);
  });

versionCommand
  .command("log <name>")
  .description("Print <name>'s full lineage, root first -- every ancestor it was branched from, down to itself.")
  .option("--dir <path>", `version store directory (default: ${DEFAULT_VERSION_STORE_DIR})`, DEFAULT_VERSION_STORE_DIR)
  .action((name: string, opts) => {
    const storeDir = path.resolve(process.cwd(), opts.dir as string);
    let chain: VersionRecord[];
    try {
      chain = lineageChain(storeDir, name);
    } catch (err) {
      console.error((err as Error).message);
      process.exit(1);
      return;
    ...
    for (const v of chain) {
      console.log(`${v.name.padEnd(24)} ${v.createdAt}${v.message ? `  ${v.message}` : ""}`);
    }
  });
