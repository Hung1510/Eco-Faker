...
// Combinatorial opener/detail/closer templates per rating tier, with the
// real product name interpolated -- same pattern as support tickets'
// message templates, replacing what used to be 2-3 fixed strings per
// rating value (~10-15 distinct reviews total across a whole dataset,
// however many thousand ratings existed). Not every slot is used for
// every rating (a 1-star review doesn't need a "closer" thanking anyone),
// kept deliberately un-uniform to avoid a new kind of repetition.
function buildReviewText(productName: string, rating: number, rng: Rng): string {
  if (rating === 5) {
    const openers = [
      `Love the ${productName}.`,
      `The ${productName} exceeded my expectations.`,
      `Really happy with the ${productName}.`,
    ];
    const details = [
      `Exactly as described, arrived quickly, and the quality is great.`,
      `Been using it daily and it's held up perfectly so far.`,
      `Worth every penny -- already recommended it to a friend.`,
    ];
    return `${rng.pick(openers)} ${rng.pick(details)}`;
  }
  if (rating === 4) {
    const openers = [
      `Solid ${productName}, a couple minor gripes.`,
      `Good value on the ${productName} overall.`,
      `Happy with the ${productName}, works as described.`,
    ];
    const details = [
      `Not perfect, but it does what I need.`,
      `Packaging could be better but the product itself is fine.`,
      `Would buy again, just wish it came in more options.`,
    ];
    return `${rng.pick(openers)} ${rng.pick(details)}`;
  }
  if (rating === 3) {
    const openers = [
      `The ${productName} is okay, does the job but nothing special.`,
      `Average experience with the ${productName} -- met expectations, no more.`,
    ];
    const details = [`It works, just not as impressive as the listing made it sound.`, `Neither great nor bad -- a middle-of-the-road pick.`];
    return `${rng.pick(openers)} ${rng.pick(details)}`;
  }
  if (rating === 2) {
    const openers = [
      `Not quite what I expected from the ${productName}'s description.`,
      `Had some issues with the ${productName}, might return it.`,
    ];
    const details = [`The quality feels lower than the price suggests.`, `Doesn't match the photos as closely as I'd hoped.`];
    return `${rng.pick(openers)} ${rng.pick(details)}`;
  }
  // rating === 1
  const openers = [`Disappointed with the ${productName}, wouldn't recommend.`, `The ${productName} arrived damaged / not as pictured.`];
  const details = [`Requesting a refund -- this isn't what I paid for.`, `Save your money and look elsewhere.`];
  return `${rng.pick(openers)} ${rng.pick(details)}`;
}
    ...
    for (const order of userOrders) {
      ...
      for (const item of order.items) {
        ...
        productRatings.push({
          ...
          orderId: order.id,
          rating,
          reviewText: rng.chance(0.6) ? buildReviewText(item.name, rating, rng) : null,
