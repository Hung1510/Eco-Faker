...
function lintOrphanedForeignKeys(dataset: Dataset, issues: LintIssue[]): void {
  ...
  const returnRequestIds = new Set(dataset.returnRequests.map((r) => r.id));
  ...
  for (const category of dataset.categories) {
    if (category.parentCategoryId !== null) {
      check("categories", category.id, "parentCategoryId", category.parentCategoryId, categoryIds);
    }
  }
  for (const product of dataset.products) {
    check("products", product.id, "categoryId", product.categoryId, categoryIds);
    check("products", product.id, "brandId", product.brandId, brandIds);
    check("products", product.id, "supplierId", product.supplierId, supplierIds);
  }

  // productId on every line item, wherever it appears -- carts, orders, and
  // shipments all embed LineItem[] rather than referencing a normalized
  // table, so this has to walk all three instead of a single FK check.
  const checkLineItems = (table: DatasetArrayKey, recordId: string, items: { productId: string }[]) => {
    for (const item of items) {
      if (!productIds.has(item.productId)) {
        issues.push({
          ...
          table,
          recordId,
          message: `${table}/${recordId} has a line item with productId "${item.productId}" that does not match any product in the catalog.`,
        });
      }
    }
  };
  for (const cart of dataset.carts) checkLineItems("carts", cart.id, cart.items);
  for (const order of dataset.orders) checkLineItems("orders", order.id, order.items);
  for (const shipment of dataset.shipments) checkLineItems("shipments", shipment.id, shipment.items);
  ...
  for (const view of dataset.productViews) {
    check("productViews", view.id, "userId", view.userId, userIds);
    check("productViews", view.id, "productId", view.productId, productIds);
  }
  for (const query of dataset.searchQueries) {
    check("searchQueries", query.id, "userId", query.userId, userIds);
    if (query.clickedProductId !== null) {
      check("searchQueries", query.id, "clickedProductId", query.clickedProductId, productIds);
    }
  }
  for (const wishlistItem of dataset.wishlistItems) {
    check("wishlistItems", wishlistItem.id, "userId", wishlistItem.userId, userIds);
    check("wishlistItems", wishlistItem.id, "productId", wishlistItem.productId, productIds);
  }
  for (const rating of dataset.productRatings) {
    check("productRatings", rating.id, "userId", rating.userId, userIds);
    check("productRatings", rating.id, "productId", rating.productId, productIds);
    check("productRatings", rating.id, "orderId", rating.orderId, orderIds);
  }

  const warehouseIds = new Set(dataset.warehouses.map((w) => w.id));
  const replenishmentOrderIds = new Set(dataset.replenishmentOrders.map((r) => r.id));

  for (const r of dataset.replenishmentOrders) {
    check("replenishmentOrders", r.id, "productId", r.productId, productIds);
    check("replenishmentOrders", r.id, "supplierId", r.supplierId, supplierIds);
    check("replenishmentOrders", r.id, "warehouseId", r.warehouseId, warehouseIds);
  }
  for (const s of dataset.stockoutPeriods) {
    check("stockoutPeriods", s.id, "productId", s.productId, productIds);
    check("stockoutPeriods", s.id, "warehouseId", s.warehouseId, warehouseIds);
    if (s.resolvedByReplenishmentId !== null) {
      check("stockoutPeriods", s.id, "resolvedByReplenishmentId", s.resolvedByReplenishmentId, replenishmentOrderIds);
    }
  }
  for (const t of dataset.warehouseTransfers) {
    check("warehouseTransfers", t.id, "productId", t.productId, productIds);
    check("warehouseTransfers", t.id, "fromWarehouseId", t.fromWarehouseId, warehouseIds);
    check("warehouseTransfers", t.id, "toWarehouseId", t.toWarehouseId, warehouseIds);
  }

  const supportTicketIds = new Set(dataset.supportTickets.map((t) => t.id));
  for (const ticket of dataset.supportTickets) {
    check("supportTickets", ticket.id, "userId", ticket.userId, userIds);
    if (ticket.orderId !== null) check("supportTickets", ticket.id, "orderId", ticket.orderId, orderIds);
    if (ticket.productId !== null) check("supportTickets", ticket.id, "productId", ticket.productId, productIds);
    if (ticket.returnRequestId !== null) {
      check("supportTickets", ticket.id, "returnRequestId", ticket.returnRequestId, returnRequestIds);
    }
  }
  for (const message of dataset.supportMessages) {
    check("supportMessages", message.id, "ticketId", message.ticketId, supportTicketIds);
  }

  for (const email of dataset.emailMessages) {
    check("emailMessages", email.id, "userId", email.userId, userIds);
    if (email.orderId !== null) check("emailMessages", email.id, "orderId", email.orderId, orderIds);
    if (email.cartId !== null) check("emailMessages", email.id, "cartId", email.cartId, cartIds);
  }
}
