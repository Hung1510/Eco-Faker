...
// Column names that are always genuinely free text regardless of how
// long any particular sampled value happens to be -- a support message
// body, a review, or a search query is meant to be full-text searchable
// even when it's short, which the length-only heuristic below can't
// know on its own (a real gap: `support_messages.body` and
// `product_ratings.review_text` both stayed under the 256-char length
// threshold in typical generated data, so they'd otherwise map to
// exact-match "keyword" instead of "text" -- exactly backwards for data
// whose stated purpose is NLP/search training).
const ALWAYS_FULL_TEXT_COLUMNS = new Set(["body", "review_text", "query", "subject"]);
...
function inferFieldType(columnName: string, values: unknown[]): EsFieldType {
  ...
  if (nonNull.length === 0) {
    ...
    if (ALWAYS_FULL_TEXT_COLUMNS.has(columnName)) return "text";
    return "keyword";
  }
