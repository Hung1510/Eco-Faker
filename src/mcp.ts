...
export function createEcoFakerMcpServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      title: "Resolve a scenario file's inherits chain into its final merged config",
      description:
        "Resolves a scenario file's full inherits chain (built-in scenario names and/or other scenario files, YAML or JSON) into a single merged config, validating it the same way generate_dataset would. Does not generate a dataset -- pass the returned config into generate_dataset's overrides to actually use it, or use this on its own to debug an authored scenario file.",
      inputSchema: {
        filePath: z.string().describe("Absolute or relative (to the server's cwd) path to the scenario file."),
      },
    },
    async (args) => {
      try {
        const absolutePath = path.resolve(args.filePath);
        const fsLoader: ScenarioFileLoader = {
        ...
        const { config, chain } = composeScenarioFile(absolutePath, fsLoader);
        resolveConfig(config);
        return textResult({ chain, config });
      } catch (err) {
        return errorResult((err as Error).message);
      }
    }
  ...
  server.registerTool(
    ...
    {
      title: "Generate a dataset with config that varies over calendar time",
      description: `Generates a dataset whose config effectively varies over calendar time within a single call -- a quiet baseline, then a demand spike, then a slow recovery, all in one merged dataset (implemented as several ordinary generate() calls, one per time segment, merged together -- not a change to the core generation loop). Pass either profileName (a built-in preset: ${Object.keys(TEMPORAL_PROFILES).join(", ")}) or an inline profile with your own segments. Returns a datasetId plus counts and a small sample, same as generate_dataset.`,
      inputSchema: {
        profileName: z.enum(Object.keys(TEMPORAL_PROFILES) as [string, ...string[]]).optional().describe("A built-in temporal profile name."),
        profile: z
          .object({
            name: z.string(),
          ...
          .optional()
          .describe("An inline profile -- provide this OR profileName, not both. Segments must be contiguous (each segment's toDaysAgo equals the next segment's fromDaysAgo) and the last segment must end at toDaysAgo: 0."),
        seed: z.number().int().optional().describe("Base seed -- each segment derives its own distinct seed from this."),
      },
    },
    async (args) => {
      try {
        if (!args.profileName && !args.profile) {
          throw new Error("Provide either profileName or profile.");
        }
        const profile = (args.profileName ? TEMPORAL_PROFILES[args.profileName] : args.profile) as TemporalProfile;
        const baseOverrides: Partial<EcoFakerConfig> = {};
        if (args.seed !== undefined) baseOverrides.seed = args.seed;
        const dataset = generateWithTemporalProfile(baseOverrides, profile);
        const datasetId = storeDataset(dataset);
        return textResult({
          datasetId,
          profileName: profile.name,
          segments: profile.segments.map((s) => ({ label: s.label, fromDaysAgo: s.fromDaysAgo, toDaysAgo: s.toDaysAgo })),
          counts: datasetCounts(dataset),
          sampleOrders: dataset.orders.slice(0, 3),
        });
      } catch (err) {
  ...
  server.registerTool(
    ...
    {
      title: "Export real OTLP/JSON distributed traces from a dataset",
      description:
        "Generates real OTLP/JSON traces from a dataset -- a 'checkout' trace per order (spanning checkout-service and payment-service resources sharing one traceId) and a 'fulfill_shipment' trace per shipment (spans exactly matching its real tracking-event timeline). Not built on the OTel Node SDK, which is for instrumenting live code, not backdating historical spans -- this constructs the real wire format directly. Returns counts and a small sample of the raw OTLP JSON rather than the full export, which can run into thousands of spans -- use the `otel-export` CLI command to write the complete export to a file.",
      inputSchema: {
        datasetId: z.string().describe("A datasetId returned by generate_dataset, generate_temporal_dataset, fuzz_dataset, or fraud_simulate."),
        seed: z.number().int().optional().describe("Seed for span/trace IDs and synthetic checkout durations (default: 1)."),
      },
    },
    async (args) => {
      try {
